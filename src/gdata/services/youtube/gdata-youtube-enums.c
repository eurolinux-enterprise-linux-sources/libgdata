...
gdata_youtube_aspect_ratio_get_type (void)
{
  ...
  if (etype == 0) {
    static const GEnumValue values[] = {
      { GDATA_YOUTUBE_ASPECT_RATIO_UNKNOWN, "GDATA_YOUTUBE_ASPECT_RATIO_UNKNOWN", "unknown" },
      { GDATA_YOUTUBE_ASPECT_RATIO_WIDESCREEN, "GDATA_YOUTUBE_ASPECT_RATIO_WIDESCREEN", "widescreen" },
      { 0, NULL, NULL }
    };
    etype = g_enum_register_static ("GDataYouTubeAspectRatio", values);
  }
  return etype;
}
